...
var API_WHITELIST = {
  ...
  apiGetOutages:1, apiSaveOutage:1, apiDeleteOutage:1,
  apiGetTesting:1, apiSaveTesting:1, apiDeleteTesting:1,
  apiGetTestingFiles:1, apiUploadTestingFile:1,
...
function setup() {
  ...
  var schema = {
    ...
    /* versionId binds an attachment to the revision it was filed under;
       testingId binds it to a testing entry instead (mutually exclusive) */
    Uploads:      ['id','ltsId','versionId','testingId','substationCode','driveFileId','fileName',
                   'caption','mimeType','createdAt'],
    ...
    Annotations:  ['id','driveFileId','substationCode','page','x','y','w','h','color','note',
                   'createdBy','createdAt'],
    /* LTS outage — an actual interruption of the scheme */
    Outages:      ['id','ltsId','substationCode','fromDateTime','toDateTime','durationMin',
                   'findings','remarks','testingId','createdBy','createdAt'],
    /* periodic LTS testing — creating one auto-files a linked Outages row */
    Testing:      ['id','ltsId','substationCode','cause','fromDateTime','toDateTime','durationMin',
                   'findings','remarks','outageId','createdBy','createdAt']
  };
...
function lastOpMap_(code) {
  ...
  readObjects_('Operations').forEach(function (o) {
    ...
    if (!t) return;
    if (!map[o.ltsId] || toDate_(t) > toDate_(map[o.ltsId])) map[o.ltsId] = t;
  });
  return map;
}
...
function apiSaveLTS(token, code, obj) {
  ...
  if (obj.id) {
    ...
    /* An EDIT corrects the record in place — it must not silently diverge
       from the current version, so mirror the change back into it. */
    var cur = currentVersion_(obj.id);
    if (cur) {
      var fields = {};
      LTS_FIELDS.forEach(function (k) { if (k !== 'lastOpDate') fields[k] = clean[k]; });
      var patch = { data: JSON.stringify(fields) };
      if (obj.customFields) {
        patch.customFields = JSON.stringify(obj.customFields);
        readObjects_('CustomFields').filter(function (x) { return x.ltsId === obj.id; })
          .forEach(function (x) { deleteById_('CustomFields', x.id); });
        obj.customFields.forEach(function (c) {
          if (!c.fieldName) return;
          appendObject_('CustomFields', { id: uuid_(), ltsId: obj.id,
            fieldName: c.fieldName, fieldValue: c.fieldValue || '', createdAt: now_() });
        });
      }
      if (obj.functionalFrom) patch.functionalFrom = obj.functionalFrom;
      updateRowById_('LTSVersions', cur.id, patch);
    }
    return { id: obj.id };
  }
...
function apiSaveLTSVersion(token, code, payload) {
  ...
  if (prev && prev.functionalFrom && toDate_(from) < toDate_(prev.functionalFrom)) {
...
function apiDeleteLTS(token, code, id) {
  ...
  readObjects_('Testing').filter(function (x) { return x.ltsId === id; }).forEach(function (x) {
    readObjects_('Uploads').filter(function (u) { return u.testingId === x.id; }).forEach(function (u) {
      try { DriveApp.getFileById(u.driveFileId).setTrashed(true); } catch (e) {}
      deleteById_('Uploads', u.id);
    });
    deleteById_('Testing', x.id);
  });
  readObjects_('Outages').filter(function (x) { return x.ltsId === id; })
    .forEach(function (x) { deleteById_('Outages', x.id); });
  deleteById_('LTS', id);
  return { ok: true };
}
...
function apiExport(token, code, format, scope, id, opts) {
  ...
  try {
    ...
    } else if (scope === 'outages') {
      var lo = readObjects_('LTS').filter(function (x) { return x.id === id; })[0];
      title = 'LTS Outages — ' + (lo ? lo.tfIct : id);
      matrix = outagesMatrix_(outagesFor_(id));
    } else if (scope === 'testing') {
      var lt2 = readObjects_('LTS').filter(function (x) { return x.id === id; })[0];
      title = 'LTS Testing — ' + (lt2 ? lt2.tfIct : id);
      matrix = testingMatrix_(testingFor_(id));
    } else if (scope === 'ltstest1') {
      var one2 = readObjects_('Testing').filter(function (x) { return x.id === id; })[0];
      if (!one2) throw new Error('Testing entry not found.');
      title = 'LTS Testing — ' + one2.cause;
      matrix = testingMatrix_([one2]);
...
function outagesMatrix_(rows) {
  var head = ['From', 'To', 'Duration (hh:mm)', 'Findings', 'Remarks', 'Auto-filed from testing', 'Logged by'];
  var body = rows.map(function (o) {
    return [fmtDt_(o.fromDateTime), fmtDt_(o.toDateTime), fmtDur_(o.durationMin),
      o.findings || '', o.remarks || '', o.testingId ? 'Yes' : 'No', o.createdBy];
  });
  return [head].concat(body);
}
function testingMatrix_(rows) {
  var head = ['Cause', 'From', 'To', 'Duration (hh:mm)', 'Findings', 'Remarks', 'Logged by'];
  var body = rows.map(function (t) {
    return [t.cause, fmtDt_(t.fromDateTime), fmtDt_(t.toDateTime), fmtDur_(t.durationMin),
...
/* Legacy records hold dd.mm.yyyy (also dd/mm/yyyy, dd-mm-yyyy).
   new Date('04.07.2026') is Invalid or misread as US m/d — parse first. */
function toDate_(v) {
...
function fmtDt_(iso) {
  ...
  var base = Utilities.formatDate(d, Session.getScriptTimeZone(), 'dd-MMM-yyyy');
  return hasTime ? base + ', ' + Utilities.formatDate(d, Session.getScriptTimeZone(), 'HH:mm') + ' hrs' : base;
}
/* 12-Jan-2026 */
function fmtD_(iso) {
  if (!iso) return '';
  var d = toDate_(iso);
  return d ? Utilities.formatDate(d, Session.getScriptTimeZone(), 'dd-MMM-yyyy') : String(iso);
}
...
function minutesBetween_(from, to) {
  var a = toDate_(from), b = toDate_(to);
  if (!a || !b) return '';
  var m = Math.round((b - a) / 60000);
  return m >= 0 ? m : '';
}

function outagesFor_(ltsId) {
  return readObjects_('Outages').filter(function (o) { return o.ltsId === ltsId; })
    .sort(function (a, b) { return toDate_(b.fromDateTime) - toDate_(a.fromDateTime); });
}

function apiGetOutages(token, code, ltsId) {
  var s = requireSession_(token);
  if (accessibleCodes_(s).indexOf(code) < 0) throw new Error('No access.');
  return outagesFor_(ltsId);
}

/* payload = {id?, ltsId, fromDateTime, toDateTime, findings, remarks} */
function apiSaveOutage(token, code, payload) {
  var s = requireSession_(token);
  if (!canEdit_(s, code)) throw new Error('Editor permission required.');
  if (!payload.fromDateTime || !payload.toDateTime) throw new Error('Outage from/to date-time are required.');
  var dur = minutesBetween_(payload.fromDateTime, payload.toDateTime);
  if (dur === '') throw new Error('Outage "to" must be after "from".');

  if (payload.id) {
    updateRowById_('Outages', payload.id, {
      fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime, durationMin: dur,
      findings: payload.findings || '', remarks: payload.remarks || ''
    });
    return { id: payload.id };
  }
  var id = uuid_();
  appendObject_('Outages', {
    id: id, ltsId: payload.ltsId, substationCode: code,
    fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime, durationMin: dur,
    findings: payload.findings || '', remarks: payload.remarks || '',
    testingId: payload.testingId || '', createdBy: s.username, createdAt: now_()
  });
  return { id: id };
}

function apiDeleteOutage(token, code, id) {
  var s = requireSession_(token);
  if (!canEdit_(s, code)) throw new Error('Editor permission required.');
  deleteById_('Outages', id);
  return { ok: true };
}

/* ================================================================== *
 *  LTS TESTING — periodic testing entries; each auto-files a linked
 *  Outages row (the scheme is genuinely off-service during a test).
 * ================================================================== */
function testingFor_(ltsId) {
  return readObjects_('Testing').filter(function (t) { return t.ltsId === ltsId; })
    .sort(function (a, b) { return toDate_(b.fromDateTime) - toDate_(a.fromDateTime); });
}

function apiGetTesting(token, code, ltsId) {
  var s = requireSession_(token);
  if (accessibleCodes_(s).indexOf(code) < 0) throw new Error('No access.');
  return testingFor_(ltsId);
}

/* payload = {id?, ltsId, cause, fromDateTime, toDateTime, findings, remarks} */
function apiSaveTesting(token, code, payload) {
  var s = requireSession_(token);
  if (!canEdit_(s, code)) throw new Error('Editor permission required.');
  if (!payload.cause || !String(payload.cause).trim()) throw new Error('Testing cause is required.');
  if (!payload.fromDateTime || !payload.toDateTime) throw new Error('Testing from/to date-time are required.');
  var dur = minutesBetween_(payload.fromDateTime, payload.toDateTime);
  if (dur === '') throw new Error('Testing "to" must be after "from".');

  if (payload.id) {
    var existing = readObjects_('Testing').filter(function (x) { return x.id === payload.id; })[0];
    updateRowById_('Testing', payload.id, {
      cause: payload.cause, fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime,
      durationMin: dur, findings: payload.findings || '', remarks: payload.remarks || ''
    });
    /* keep the linked outage's window in sync */
    if (existing && existing.outageId) {
      updateRowById_('Outages', existing.outageId, {
        fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime, durationMin: dur,
        findings: payload.findings || '', remarks: 'Auto-filed from LTS testing: ' + (payload.cause || '')
      });
    }
    return { id: payload.id };
  }
  ...
  var id = uuid_();
  appendObject_('Testing', {
    id: id, ltsId: payload.ltsId, substationCode: code, cause: payload.cause,
    fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime, durationMin: dur,
    findings: payload.findings || '', remarks: payload.remarks || '',
    outageId: '', createdBy: s.username, createdAt: now_()
  });

  /* every testing entry takes the scheme off service — file the outage automatically */
  var outageId = uuid_();
  appendObject_('Outages', {
    id: outageId, ltsId: payload.ltsId, substationCode: code,
    fromDateTime: payload.fromDateTime, toDateTime: payload.toDateTime, durationMin: dur,
    findings: payload.findings || '',
    remarks: 'Auto-filed from LTS testing: ' + payload.cause,
    testingId: id, createdBy: s.username, createdAt: now_()
  });
  updateRowById_('Testing', id, { outageId: outageId });

  return { id: id, outageId: outageId };
}

function apiDeleteTesting(token, code, id) {
  var s = requireSession_(token);
  if (!canEdit_(s, code)) throw new Error('Editor permission required.');
  /* remove the outage this testing entry auto-filed, and any attachments */
  readObjects_('Outages').filter(function (o) { return o.testingId === id; })
    .forEach(function (o) { deleteById_('Outages', o.id); });
  readObjects_('Uploads').filter(function (u) { return u.testingId === id; }).forEach(function (u) {
    try { DriveApp.getFileById(u.driveFileId).setTrashed(true); } catch (e) {}
    deleteById_('Uploads', u.id);
  });
  deleteById_('Testing', id);
  return { ok: true };
}

/* ---- attachments filed under a testing entry ---- */
function apiGetTestingFiles(token, code, testingId) {
  var s = requireSession_(token);
  if (accessibleCodes_(s).indexOf(code) < 0) throw new Error('No access.');
  return readObjects_('Uploads').filter(function (u) { return u.testingId === testingId; })
    .sort(function (a, b) { return toDate_(b.createdAt) - toDate_(a.createdAt); })
    .map(function (u) { return { id: u.id, driveFileId: u.driveFileId, fileName: u.fileName,
      caption: u.caption, mimeType: u.mimeType, createdAt: u.createdAt }; });
}
function apiUploadTestingFile(token, code, testingId, base64, fileName, mimeType, caption) {
  var s = requireSession_(token);
  if (!canEdit_(s, code)) throw new Error('Editor permission required.');
  var file = saveToDrive_(code, { base64: base64, mimeType: mimeType, fileName: fileName });
  var id = uuid_();
  appendObject_('Uploads', { id: id, ltsId: '', versionId: '', testingId: testingId,
    substationCode: code, driveFileId: file.getId(),
    fileName: fileName, caption: caption || fileName, mimeType: mimeType, createdAt: now_() });
  return { id: id, driveFileId: file.getId() };
}
...
function migrateSchema() {
  ...
  if (addColumnIfMissing_('Uploads', 'testingId', 'substationCode')) added.push('Uploads.testingId');
